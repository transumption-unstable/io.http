(ns yegortimoshenko.io.http
  ...
  (:import (java.io CharArrayReader File InputStream IOException
                    OutputStream OutputStreamWriter Reader StringReader)
...
(defn ^:private normalize-headers [map]
  (into {} (for [[k v] map] (if k [(header->keyword k) v]))))

(defn request
  "Sends a HTTP request and returns a response map that contains
  body (InputStream), headers (map from keyword to list of strings),
  ...
  Parameters:
    ...
    :headers  header fields, map from keywords to collection of strings
    ...
    :method   keyword that represents HTTP method to be used; can be one of
              :GET (default), :POST, :HEAD, :PUT, :DELETE, :TRACE, :OPTIONS
    :url      HTTP/HTTPS URL as String or java.net.URL (required)
  ...
  [{:keys [body encoding headers length method url] :as request}]"
  (let [conn ^HttpURLConnection (.openConnection (if (instance? URL url) url (URL. url)))]
    (when body
